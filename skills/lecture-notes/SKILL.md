...
Phase 4: Semantic Seg.   → strategic review agent
Phase 5: Segment Prep.    → lecture-fusion.py + lecture-clips.py
Phase 6: AI Scouting     → vision-capable agent (video+keyframe)
Phase 7: Selective OCR   → vision-capable agent (parallel)
Phase 8: Composition     → Orchestrator + implementation agent
Phase 9: Review          → strategic review agent
```
...
**Delegate to vision-capable agent** with the 6 frames:
...
If vision-capable agent returns multiple archetypes, average their thresholds
as the starting value. The retry loop will find the correct threshold.

**If not already specified:** Ask user for output directory and language.

**Capture domain terms** from the vision-capable agent response. Format as a whisper `--prompt`
...
```bash
# Format after vision-capable agent returns terms:
...
~/.agents/scripts/transcribe "$video_abs" --language LANG \
...
```bash
python3 ~/.agents/scripts/lecture-scenes.py video.mp4 -t THRESHOLD --min-duration VALUE -o scenes.json
...
**Delegate to strategic review agent.** Send full SRT (1M-token context models handle this easily).
...
python3 ~/.agents/scripts/lecture-fusion.py \
...
```bash
video_abs="$(realpath video.mp4)"
python3 ~/.agents/scripts/lecture-clips.py segments.json "$video_abs" --output-dir clips/
```
...
**Delegate to vision-capable agent. Send all sections in PARALLEL calls.** Each section has its own
...
**Delegate to vision-capable agent. Send ALL `needs_ocr` slides in PARALLEL calls.**
...
**Empty result handling:** Some vision-capable agent OCR calls return empty results (~30%
...
**Agent:** Orchestrator (integration). Implementation agent for per-section parallel drafting if 10+ sections.
...
**Delegate to strategic review agent:**
...
| 4 | strategic review agent | Structured JSON from transcript |
| 5 | Orchestrator | Runs lecture-fusion.py + lecture-clips.py |
| 6 | vision-capable agent | Vision + text fusion (audio+visual together) |
| 7 | vision-capable agent (parallel) | OCR, independently parallelizable |
| 8 | Orchestrator + implementation agent | Integration + per-section drafting |
| 9 | strategic review agent | Quality review |
...
3. **Phase 6 is parallel:** Send all per-segment keyframes + video clips to vision-capable agent simultaneously. Each segment has its own keyframe and video clip — calls are independent with no shared state.
4. **Phase 7 is parallel:** Send all `needs_ocr` slides to vision-capable agent simultaneously. Independent images, no cross-contamination risk.
...
**Skill dependencies:** Orchestrator: `audio-analysis` (transcribe details). Vision-capable agent (phases 6-7): `video-analysis` (model options), `lecture-notes` (output field spec).
...
This skill uses generic agent role names. When used with specific harnesses, map:
- "vision-capable agent" → your harness's multimodal/vision subagent
- "strategic review agent" → your harness's review/planning subagent  
- "implementation agent" → your harness's code-execution subagent

Harness-specific overlays can provide precise agent mappings.
